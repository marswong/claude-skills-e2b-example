#!/usr/bin/env python3
"""
Sora 2 Image-to-Video 模块
整合 KIE.AI 的 Sora 2 Image-to-Video API 到 ChatGPT Skill
...
使用方法:
    ...
    client = SoraImg2VideoClient(api_key="your-kie-api-key")
...
class SoraImg2VideoClient:
    """Sora 2 Image-to-Video 客户端"""
    ...
    def generate(
        ...
        Args:
            image_url: 图片URL（单个或列表）。必须是公网可访问的 JPEG/PNG/WebP，每张最大10MB
            prompt: 描述希望的视频动作（最多10000字符）
        ...
        if not image_urls:
            raise ValueError("至少需要提供一个图片URL")
        ...
        print(f"正在创建图片转视频任务...")
        print(f"图片: {image_urls[0][:50]}..." if len(image_urls[0]) > 50 else f"图片: {image_urls[0]}")
    ...
    def batch_generate(
        ...
        Args:
            items: 列表，每项包含 {"image_url": "...", "prompt": "..."}
            aspect_ratio: 宽高比
            duration: 时长
        ...
        for i, item in enumerate(items, 1):
            print(f"\n[{i}/{len(items)}] 处理: {item.get('prompt', '')[:50]}...")
            try:
                url = self.generate(
                    image_url=item["image_url"],
                    prompt=item.get("prompt", "让图片动起来"),
                    aspect_ratio=aspect_ratio,
                    duration=duration
                )
                results.append({**item, "url": url, "success": True})
            except Exception as e:
                results.append({**item, "error": str(e), "success": False})
        return results
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sora 2 Image-to-Video 图片转视频工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 基本用法 - 将图片转为视频
  python3 sora_img2video.py --image "https://example.com/cat.jpg" --prompt "猫站起来走动"

  # 生成竖向视频 (适合手机/抖音)
  python3 sora_img2video.py --image "https://example.com/photo.jpg" --prompt "人物微笑" --aspect portrait

  # 生成15秒视频
  python3 sora_img2video.py --image "https://example.com/scene.jpg" --prompt "云朵缓缓移动" --duration 15

  # 保存视频到本地
  python3 sora_img2video.py --image "https://example.com/img.jpg" --prompt "动起来" --output video.mp4

  # 使用本地图片（需先上传到可访问的URL）
  # 建议使用图床服务如 imgur, cloudinary 等
"""
    )

    parser.add_argument("--image", "-img", required=True, help="图片URL (公网可访问)")
    parser.add_argument("--prompt", "-p", required=True, help="描述希望的视频动作")
